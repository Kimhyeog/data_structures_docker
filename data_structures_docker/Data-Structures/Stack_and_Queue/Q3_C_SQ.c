...
{
	// stack 유효성 검사
	if (s == NULL || isEmptyStack(s) || s->ll.size <= 1)
		return 0;
	...
	int isConsecutive = 1; // 최종 결과 저장

	while (isEmptyStack(s) && s->ll.size > 1)
	{
		// 첫번째 원소 꺼냄
		int val1 = pop(s);
		// 꺼내고 비었는지 검사
		if (isEmptyStack(s)) // 비어있다면,
		{
			isConsecutive = 0;
			// 복구를 위해 temp에 저장
			push(&temp, val1);
		...
		if (abs(val1 - val2) != 1) // 둘의 차이 1이 아닐시
			isConsecutive = 0;	   // 기록

		// 순서 보존을 위해 temp에 push (val2 먼저, val1 나중)
		push(&temp, val2);
		push(&temp, val1);
	}

	// 3. temp에서 다시 pop → 원래 스택에 push
