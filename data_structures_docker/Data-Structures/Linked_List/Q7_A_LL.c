...
void RecursiveReverse(ListNode **ptrHead)
{
	...
	// 3. 후위적으로 끝 노드로 *ptrHead -> NULL(*reset) 도달 경우
	// 	*ptrHead의 노드의 next노드의 next링크를 *ptrHead로 연결
	(*ptrHead)->next->next = *ptrHead;
	// 해당 *ptrNode 링크 해제 (이유: 이새끼가 끝노드 될거라서)
	(*ptrHead)->next = NULL;

	// 새로운 head로 갱신 (맨 마지막 호출에서만 실행됨)
	*ptrHead = rest;
}
